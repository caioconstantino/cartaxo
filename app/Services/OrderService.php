...
class OrderService
{
     ...
     * @throws Exception
     */
    public function updateShippingInfo(Order $order, array $shippingData): Order
{
    try {
        $order->codigoObjeto = $shippingData['codigoObjeto'] ?? $order->codigoObjeto;
        $order->idPrePostagem = $shippingData['idPrePostagem'] ?? $order->idPrePostagem;
        $order->idRecibo = $shippingData['idRecibo'] ?? $order->idRecibo;
        
        $order->save();
        return $order;
    } catch (Exception $exception) {
        Log::error("Erro ao salvar informações de envio: " . $exception->getMessage());
        throw new Exception($exception->getMessage(), 422);
    }
}
