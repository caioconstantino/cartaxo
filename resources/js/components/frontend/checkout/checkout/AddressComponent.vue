<template>
    ...
    <div v-if="show" class="mb-6 rounded-2xl shadow-card">
        <div class="flex flex-wrap items-center justify-between gap-3 p-4 border-b border-gray-100">
            <h4 class="font-bold capitalize text-white">{{ title }}</h4>
        ...
        <div v-if="shippingData.valorFreteJADLOG"
            class="mt-6 p-4 border-t border-gray-100">
            <h5 class="font-bold text-white text-base mb-4">Selecione a forma de entrega</h5>
            <div class="flex flex-col gap-4">
                <label class="flex items-center gap-3 cursor-pointer">
                    <input type="radio" name="shipping" value="SEDEX" v-model="selectedShipping"
                        @change="updateShippingCharge(shippingData.valorFreteSEDEX, '03220')" class="h-5 w-5">
                    <span class="text-sm font-medium text-white">
                        LOGGI - (R$ {{ shippingData.valorFreteSEDEX.toFixed(2) }}, {{ shippingData.prazoEntregaSEDEX }}
                        dias úteis)
                    </span>
                </label>
                <!--<label class="flex items-center gap-3 cursor-pointer">
                    <input type="radio" name="shipping" value="PAC" v-model="selectedShipping"
                        @change="updateShippingCharge(shippingData.valorFretePAC, '03298')" class="h-5 w-5">
                    <span class="text-sm font-medium text-white">
                        PAC - (R$ {{ shippingData.valorFretePAC.toFixed(2) }}, {{ shippingData.prazoEntregaPAC }} dias
                        úteis)
                    </span>
                </label>-->
                <label class="flex items-center gap-3 cursor-pointer">
                    <input type="radio" name="shipping" value="JADLOG" v-model="selectedShipping"
                        @change="updateShippingCharge(shippingData.valorFreteJADLOG, '00001')" class="h-5 w-5">
                    <span class="text-sm font-medium text-white">
                        JADLOG - (R$ {{ shippingData.valorFreteJADLOG.toFixed(2) }}, {{ shippingData.prazoEntregaJADLOG }} dias
                        úteis)
                    </span>
                </label>
            </div>
...
export default {
    ...
    methods: {
        ...
        async activeAddress(address) {
            this.activeAddressId = address.id;
            this.method(address);
            console.log(`Selecionado: ${address.zip_code}`);

            try {
                const origem = "01021000"; // CEP de origem
                const destino = address.zip_code; // CEP do cliente

                // Obtém os valores dos produtos no carrinho
                const carts = this.$store.getters['frontendCart/lists'];

                // Soma os valores de peso, altura, largura e comprimento de todos os produtos
                const pesoTotal = carts.reduce((total, cart) => total + (parseFloat(cart.weight) || 0) * cart.quantity, 0);
                const comprimentoTotal = carts.reduce((total, cart) => total + (parseFloat(cart.length) || 0) * cart.quantity, 0);
                const alturaTotal = carts.reduce((total, cart) => total + (parseFloat(cart.height) || 0) * cart.quantity, 0);
                const larguraTotal = carts.reduce((total, cart) => total + (parseFloat(cart.width) || 0) * cart.quantity, 0);

                // Monta os parâmetros para a request
                ...
                // Valida e preenche os dados de frete no componente
                if (data.valorFreteSEDEX && data.valorFretePAC) {
                    this.shippingData = {
                        valorFreteSEDEX: parseFloat(data.valorFreteSEDEX.replace(",", ".")),
                        prazoEntregaSEDEX: data.prazoEntregaSEDEX,
                        valorFretePAC: parseFloat(data.valorFretePAC.replace(",", ".")),
                        prazoEntregaPAC: data.prazoEntregaPAC,
                        valorFreteJADLOG: data.valorFreteJADLOG,
                        prazoEntregaJADLOG: data.prazoEntregaJADLOG,
                    };
                    console.log("Dados de frete atualizados:", this.shippingData);
                } else {
        ...
        updateShippingCharge(valorFrete, selectedShipping){
            this.$store.commit("frontendCart/UPDATE_SHIPPING_CHARGE", parseFloat(valorFrete));
            this.$store.commit("frontendCart/UPDATE_SHIPPING_METHOD", selectedShipping);
            console.log(`Frete selecionado: R$ ${valorFrete},${selectedShipping}`);
        },
        // this.$store.commit("frontendCart/UPDATE_SHIPPING_CHARGE", parseFloat(data.valorFrete));
        showTarget(targetID, addClass) {
