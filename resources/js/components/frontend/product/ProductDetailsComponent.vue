        ...
        <div class="container">
            <div class="row">
                ...
                <div class="col-12 sm:col-6 lg:col-7 lg:pl-10">
                    <h2 class="text-primary text-3xl sm:text-4xl font-bold capitalize mb-5">{{ product.name }}</h2>
                    ...
                    <dl class="flex flex-wrap items-center gap-x-6 gap-y-3 mb-8">
                        <dt class="capitalize text-lg font-semibold text-white">{{ $t('label.quantity') }}:</dt>
                        <dd class="flex items-center gap-6">
                            ...
                            <div v-if="!initialVariations.length || selectedVariation != null">
                                <p v-if="temp.stock > 0" class="capitalize text-white">
                                    {{ $t('label.available') }}:
                                    <b>({{ temp.stock }}) </b>
                                    {{ product.unit }}
                                </p>
                    ...
                    <div class="flex flex-wrap items-center gap-8 mb-10">
                        <button @click.prevent="buyNow" :disabled="enableAddToCardButton" type="button"
                            ...
                            <span class="whitespace-nowrap font-bold">Comprar Agora</span>
                        </button>
                    ...
                    <div class="flex flex-col gap-4 w-full max-w-md">
                        <label for="cep" class="text-white font-semibold">Calcular Frete e Prazo</label>
                        <div class="flex items-center gap-4">
                            <input id="cep" type="text" v-model="cep" placeholder="Digite seu CEP"
                                class="flex-1 px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-primary text-white">
                            <button @click="calculateFreight" type="button"
                                class="px-6 py-2 bg-primary text-white rounded-lg hover:bg-primary-dark transition-all">
                                Calcular
                            </button>
                        </div>
                        <div v-if="freightJadlogValue !== null" class="text-white font-semibold">
                            LOGGI: <span class="text-primary">R$ {{ freightSedexValue | currency }} ({{ prazoEntregaSEDEX }} Dias
                                Úteis)</span>
                                <br>
                            <!-- PAC: <span class="text-primary">R$ {{ freightPacValue | currency }} ({{ prazoEntregaPAC }} Dias
                                Úteis)</span>
                                <br> -->
                                 JADLOG: <span class="text-primary">R$ {{ freightJadlogValue | currency }} ({{ prazoEntregaJadlog }} Dias
                                Úteis)</span>
                        </div>
                    ...
                    <div class="rounded-[32px] border border-[#D9DBE9]">
                        <nav class="flex flex-wrap items-center p-4 sm:py-6 sm:px-8 gap-3 sm:gap-6">
                            <button type="button"
                                ...
                                class="text-primary tab-btn active text-sm sm:text-base font-bold leading-5 capitalize py-2 sm:py-3.5 px-5 sm:px-8 rounded-full border border-[#D9DBE9]">
                                {{ $t('label.details') }}
                            </button>

                            <button type="button"
                                @click.prevent="multiTargets($event, 'tab-btn', 'tab-div', 'tab_videos')"
                                class="text-primary tab-btn text-sm sm:text-base font-bold leading-5 capitalize py-2 sm:py-3.5 px-5 sm:px-8 rounded-full border border-[#D9DBE9]">
                                {{ $t('label.videos') }}
                            </button>
                        ...
                        <div id="tab_details" class="tab-div active p-4 sm:p-8 sm:pt-6 border-t border-[#D9DBE9]">
                            <h3 class="capitalize text-2xl sm:text-3xl font-bold mb-4 text-primary">
                                {{ $t('label.product_details') }}
                            </h3>
                            <span class="forcar-branco">
                                <div class="text-description" style="color: white !important;" v-html="product.details"></div>
                            </span>
                        </div>
...
export default {
    ...
    data() {
        return {
            ...
            freightSedexValue: null, // Adicionado aqui
            freightPacValue: null, // Adicionado aqui
            freightJadlogValue: null,
            prazoEntregaSEDEX: null,
            prazoEntregaPAC: null,
            prazoEntregaJadlog: null,
            error: '', // Adicionado para mensagens de erro
        };
    },
    computed: {
        setting: function () {
    ...
    methods: {
        ...
        async calculateFreight() {
            this.error = '';
            this.freightSedexValue = null;
            this.freightPacValue = null;
            this.prazoEntregaSEDEX = null;
            this.prazoEntregaPAC = null;
            this.freightJadlogValue = null;
            this.prazoEntregaJadlog = null;
            ...
            try {
                const origem = "01021000"; // CEP de origem (fixo)
                const destino = this.cep; // CEP inserido pelo usuário
                const peso = 0.30; // Peso do pacote (em kg)
                const comprimento = 20; // Comprimento (em cm)
                const altura = 10; // Altura (em cm)
                const largura = 15; // Largura (em cm)

                console.log(`Calculando frete para CEP ${this.cep} com os seguintes parâmetros:`);
                console.log({ origem, destino, peso, comprimento, altura, largura });
                ...
                const response = await fetch(`https://correios.constantinocode.com.br/frete.php?${params.toString()}`, {
                    method: "GET",
                });

                if (!response.ok) {
                    throw new Error(`Erro na request: ${response.statusText}`);
                }

                const data = await response.json();

                if (data && typeof data === 'object' && data.valorFreteSEDEX && !isNaN(parseFloat(data.valorFreteSEDEX))) {
                    this.freightSedexValue = parseFloat(data.valorFreteSEDEX);
                    this.prazoEntregaSEDEX = parseFloat(data.prazoEntregaSEDEX);

                    this.freightPacValue = parseFloat(data.valorFretePAC);
                    this.prazoEntregaPAC = parseFloat(data.prazoEntregaPAC);

                    this.freightJadlogValue = parseFloat(data.valorFreteJADLOG);
                    this.prazoEntregaJadlog = parseFloat(data.prazoEntregaJADLOG);
                } else {
                    this.error = 'Não foi possível calcular o frete. Tente novamente.';
                    console.error("Resposta inválida do servidor:", data);
                }
            } catch (error) {
                this.error = 'Erro ao calcular o frete. Verifique sua conexão ou tente mais tarde.';
                console.error("Erro ao calcular o frete:", error.message, error.stack);
            }
        },
        readMore: function () {
            ...
            if (typeof this.$route.params.slug !== "undefined") {
                ...
                this.$store.dispatch("frontendProduct/show", this.props.search).then((res) => {
                    ...
                    this.temp = {
                        ...
                        price: res.data.data.price,
                        oldPrice: res.data.data.old_price,
                        weight: res.data.data.weight, // Certifique-se de incluir o peso
                        height: res.data.data.height, // Certifique-se de incluir a altura
                        width: res.data.data.width, // Certifique-se de incluir a largura
                        length: res.data.data.length, // Certifique-se de incluir o comprimento
                        totalPrice: res.data.data.price
                    };
        ...
        addToCart: function () {
            ...
            this.productArray = {
                ...
                weight: this.temp.weight, // Certifique-se de incluir o peso
                height: this.temp.height, // Certifique-se de incluir a altura
                width: this.temp.width, // Certifique-se de incluir a largura
                length: this.temp.length, // Certifique-se de incluir o comprimento
        ...
        buyNow: function () {
            ...
            const finalizeCheckout = () => {
                // Vá para a rota de checkout (ajuste a rota abaixo conforme sua aplicação)
            ...
            if (this.selectedVariation) {
                this.$store.dispatch("frontendProductVariation/ancestorsToString", this.selectedVariation.id).then((res) => {
                    ...
                    console.error("Erro ao carregar variação:", err);
                });
