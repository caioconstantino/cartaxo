    ...
    <div class="db-card p-4">
      <div class="flex flex-wrap gap-y-5 items-end justify-between">
        <div>
          <div class="flex flex-wrap items-start gap-y-2 gap-x-6 mb-5">
            ...
            <div class="flex items-center gap-2 mt-1.5">
              <span
                :class="
                  'text-sm capitalize h-5 leading-5 px-2 rounded-3xl text-[#FB4E4E] bg-[#FFDADA]' +
              ...
              <span
                :class="
                  'text-sm capitalize px-2 rounded-3xl ' +
        ...
        >
          ...
          <button
            type="button"
            @click="changeStatus(enums.orderStatusEnum.CONFIRMED)"
        ...
        >
          <div class="relative">
            ...
            <i
              class="lab lab-line-chevron-down lab-font-size-16 absolute top-1/2 right-3.5 -translate-y-1/2 text-primary"
            ></i>
          </div>
          ...
          >
            ...
            >
              <option
                v-for="orderStatus in enums.orderStatusPickupObject"
                :value="orderStatus.value"
              >
                {{ orderStatus.name }}
              </option>
          ...
          <button
            ...
            class="flex items-center justify-center text-white gap-2 px-4 h-[38px] rounded shadow-db-card bg-[#2AC769]"
          >
            <i class="lab lab-line-tag"></i>
            <span class="text-sm capitalize text-white">Imprimir Etiqueta</span>
          </button>
        </div>
      </div>
    </div>
              ...
              >
                <div class="flex items-center gap-3 relative">
                  ...
                  <div class="flex-auto overflow-hidden">
                    <h4
                      :class="!product.variation_names ? 'mb-4' : ''"
    ...
    <div class="row">
      ...
      <div
        ...
        v-if="
          ...
          order.status === enums.orderStatusEnum.PENDING
        "
      ...
      >
        <div class="db-card">
          <div class="db-card-header">
            ...
            <h3
              class="db-card-title"
              v-if="address.address_type === enums.addressTypeEnum.BILLING"
            >
      ...
      <div
        class="col-12"
        v-else-if="order.order_type === enums.orderTypeEnum.PICK_UP"
      >
        <div class="db-card">
          <div class="db-card-header">
          ...
          <div class="db-card-body">
            ...
            <ul class="flex flex-col gap-3 py-4 border-t border-[#EFF0F6]">
              ...
              <li class="flex items-center gap-2.5">
                <i class="lab lab-line-location lab-font-size-14"></i>
                <span class="text-xs">
                  <span v-if="outletAddress.address"
                    >{{ outletAddress.address }},</span
                  >
                  <span v-if="outletAddress.city"
                    >{{ outletAddress.city }},</span
                  >
                  <span v-if="outletAddress.state"
                    >{{ outletAddress.state }},</span
                  >
                  <span v-if="outletAddress.country"
                    >{{ outletAddress.country }},</span
                  >
                  <span v-if="outletAddress.zip_code">{{
...
export default {
  ...
  async mounted() {
    ...
    try {
      ...
      this.orderData = res.data.data; // garante que orderData existe
      this.order_status = res.data.data.status;
      this.payment_status = res.data.data.payment_status;
    } catch (err) {
      alertService.error("Erro ao carregar dados do pedido");
    } finally {
...
async generateNfe() {
  try {
    if (!this.orderData) {
      throw new Error("Dados do pedido não carregados");
    }

    this.loading.isActive = true;

    // --------------------------
    // 1. CHAMAR ENDPOINT LARAVEL PARA EMITIR E ENVIAR NFE
    // --------------------------
    const response = await axios.post('bling/nfe/emitir-enviar', {
      order_id: this.orderData.id
    });

    const { nfeId, numero, serie, xml } = response.data;

    console.log("NFe gerada e autorizada:", response.data);

    // Salvar dados da NFe localmente, se necessário
    this.orderData.nfe = {
      id: nfeId,
      numero,
      serie,
    ...
    alertService.success("NFe emitida e autorizada com sucesso!");
    
    return response.data;
  } catch (err) {
    console.error("Erro ao gerar e autorizar NFe:", err.response?.data || err.message);
      ...
      appService
        ...
        .then((res) => {
          try {
            this.loading.isActive = true;
            this.$store
              .dispatch("onlineOrder/changeStatus", {
    ...
    changeStatus: async function (status) {
      ...
      try {
        // Certifica que orderData existe
        if (!this.orderData || !this.orderData.user) {
          throw new Error("Dados do pedido não carregados.");
        }

        // Atualiza status do pedido
        const updateRes = await this.$store.dispatch(
          "onlineOrder/changeStatus",
          {
            id: this.$route.params.id,
            status: status,
          }
        );
        this.order_status = updateRes.data.data.status;

        // Verifica CPF ou CNPJ do cliente
        const cpfcnpj = this.orderData.user.cpfcnpj.replace(/\D/g, "");
        let tipoPessoa = null;

        if (cpfcnpj.length === 11) tipoPessoa = "F";
        else if (cpfcnpj.length === 14) tipoPessoa = "J";
        else throw new Error("Documento inválido");

        // Salva tipoPessoa no objeto antes de gerar NFe
        this.orderData.user.tipoPessoa = tipoPessoa;

        // Gera a nota fiscal
        await this.generateNfe();
    ...
    changePaymentStatus: function (e) {
      try {
        this.loading.isActive = true;
        this.$store
          .dispatch("onlineOrder/changePaymentStatus", {
            id: this.$route.params.id,
